import React from 'react'

function Main() {
  return (
    <div className='mt-8'>
        <p className='w-full md:w-[90%] lg:w-[80%] text-[10px] md:text-base lg:text-xl'>
            Wix is leading the way with a cloud-based development platform for users worldwide. Wix was founded on the belief that the internet
        should be accessible to everyone to develop, create and contribute. Through free and premium subscriptions, 
        Wix empowers millions of businesses, organizations,artists and individuals to take their businesses, brands 
        and workflow online.
        </p>

        <div className='flex justify-center items-center mt-12 bg-zinc-200 h-[10rem] md:h-[20rem] lg:h-[32rem]'>
            <img className='h-[90%] w-[90%] rounded' 
            src='https://images.pexels.com/photos/389819/pexels-photo-389819.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1'
            />
        </div>
        
    </div>
  )
}

export default Main